...
//write a number using images of digits
void drawNum(DrawingWindow &window, int num, int x, int y, int h){
	int width = (int)( (float)(h) / 1.5f );
	std::string stringNum = std::to_string(num);
	for (int i = 0; i < stringNum.size(); i++) {
		window.drawIMG("assets/numbers/" + std::string(1, stringNum[i]) + ".png", x + i*width, y, width, h);
	}
}
...
void draw(DrawingWindow &window) {
	...
	for(int i = 0; i < map.arcs.size(); i++){
		...
		//drawNum(window, a.length(map.routeMode), mx, my, 30);
	}
	//draw points
	for(int i = 0; i < map.points.size(); i++){
